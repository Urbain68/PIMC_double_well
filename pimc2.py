...
# -----------------------------
T_default = 1.0/3.0            # température (en unités réduites V0/kB)
...
N_REPLICATES = 10**1           # nombre de réplicats global par défaut
...
def plot_energy_with_acceptance(epsilon_list, P=None, T=None,
    ...
    for eps in epsilon_list:
        ...
        line, = plt.plot(steps, Vt, label=f"ε={eps}")
    ...
    for eps in epsilon_list:
        ...
        line, = plt.plot(steps, mean_curve, label=f"ε={eps} mean")
        plt.fill_between(steps, mean_curve-std_curve, mean_curve+std_curve,
                         alpha=0.25, color=line.get_color())
        plt.text(steps[-1], mean_curve[-1], f"{np.mean(acc_rates)*100:.1f}%", fontsize=9,
...
def questions_7_8_vary_T_and_C():
    ...
    for T in T_list:
        ...
        plot_rho_and_barrier(out['beads_concat'], out['centroids_concat'], T=T, xlim=(-3,3),
    ...
    for C in [0.3, 0.6]:
        # relation indicatrice : V0_proportionnel = 1/C (exemple uniquement — vérifier vraie relation)
        V0_mod = 1.0 / C
        ...
        V_backup = V
        V = V_local
        out = run_ensemble(10, P=16, n_steps=20000, epsilon=0.005, T=0.4, K=None)
